...
public class ErroProduto {
	...
	static final String QTD_NAO_ACEITA = "Quantidade de produto não aceita";
	...
	public static ResponseEntity<?> erroProdutoJaCadastrado(ProdutoDoCarrinhoDTO produto) {
		return new ResponseEntity<CustomErrorType>(new CustomErrorType(String.format(ErroProduto.PRODUTO_JA_CADASTRADO,
				produto.getProduto().getNome(), produto.getProduto().getFabricante())), HttpStatus.CONFLICT);
	}
	
	public static ResponseEntity<?> erroQtdProdutoNaoAceita() {
		return new ResponseEntity<CustomErrorType>(new CustomErrorType(ErroProduto.QTD_NAO_ACEITA),
				HttpStatus.CONFLICT);
	}
}
